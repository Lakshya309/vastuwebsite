...
export async function POST(req: NextRequest) {
  try {
    ...
    if (uploadError) {
      ...
      return NextResponse.json({ message: "Failed to upload file to storage", error: uploadError.message }, { status: 500 });
    }
    ...
    if (updateError) {
      ...
      return NextResponse.json({ message: "Failed to save file URL to database", error: updateError.message }, { status: 500 });
    }

    return NextResponse.json({ message: "File uploaded and project updated successfully", url: floorPlanUrl }, { status: 200 });
  } catch (error: any) {
