...
export default function ReportPage() {
  ...
  const [reportAnalyses, setReportAnalyses] = useState<
  ...
  useEffect(() => {
    if (
      !project ||
      !project.boundary_normalized ||
      project.boundary_normalized.length < 3
    )
      return;
    ...
    for (const obj of placedObjects) {
      ...
      for (const region of zones16) {
        if (isPointInPolygon(obj.centroid, region.polygon)) {
          zone = region.name;
          break;
        }
      }
      ...
      newAnalyses[obj.id] = analyzeObject(obj.object_type, zone, devta);
    }
    setReportAnalyses(newAnalyses);
  }, [project, placedObjects, liveNorthDirection, devtaRegions, zones16]);

  // --- DERIVED DATA FOR CHARTS ---
  const analysisValues = Object.values(reportAnalyses);
  const goodObjectsCount = analysisValues.filter(
    (a) => a.status === "GOOD" || a.status === "EXCELLENT",
  ).length;
  const badObjectsCount = analysisValues.length - goodObjectsCount;

  const overallScore =
    analysisValues.length > 0
      ? (goodObjectsCount / analysisValues.length) * 100
      : 0;
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {placedObjects.map((obj) => {
                ...
                const isBad =
                  analysis.status === "BAD" || analysis.status === "CRITICAL";
                return (
                  <tr key={obj.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <span
                        ...
                        style={{
                          backgroundColor: `${
                            isBad ? COLORS.bad : COLORS.good
                          }20`,
                          color: isBad ? COLORS.bad : COLORS.good,
                        }}
                      >
                        {analysis.status}
                      </span>
