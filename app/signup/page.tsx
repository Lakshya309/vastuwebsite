...
function SignupContent() {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      setError(err.message || "Failed to initiate registration connection.");
          ...
          <div className="relative z-10">
            ...
            <div className="space-y-6">
              ...
              <form onSubmit={handleSignup} className="space-y-6">
                    ...
                    <div className="px-4 mt-2">
                      <div className="flex gap-1 h-1 w-full rounded-full overflow-hidden bg-gray-200">
                        {[0, 1, 2, 3].map((index) => (
                          <div 
                            key={index} 
                            className={`h-full flex-1 transition-all duration-300 ${index < strength ? strengthColors[Math.max(0, strength - 1)] : 'bg-transparent'}`} 
                          />
                        ))}
                      </div>
                      <p className="text-[9px] text-gray-400 mt-1 uppercase tracking-wider font-bold">
                ...
                <div className="space-y-2">
                  <label htmlFor="confirmPassword" className="block text-[9px] font-bold text-gray-400 uppercase tracking-widest ml-4">
                    Confirm Access Key
                  </label>
                  <input
                    type="password"
                ...
                <button
                  ...
                  className="w-full py-5 bg-primary text-white rounded-2xl text-[10px] font-bold uppercase tracking-[0.3em] shadow-xl shadow-primary/20 hover:scale-[1.02] active:scale-95 transition-all duration-300 disabled:opacity-75 disabled:cursor-not-allowed flex items-center justify-center gap-4 px-6"
                >
                  {isLoading ? (
                    <>
